...
def set_dns_resolvers(checker, nameservers):
    """
    Устанавливает новые DNS-резолверы.
    nameservers – список IP-адресов.
    Настраивает resolvectl (если активен), перезаписывает /etc/resolv.conf,
    затем перезапускает systemd-resolved для применения изменений.
    ...
    if out.strip() == 'active':
        lines.append("Обнаружен systemd-resolved. Настраиваем resolvectl и обновляем /etc/resolv.conf.")

        # Получаем список интерфейсов (кроме lo)
        ...
        if interfaces:
            # Сбрасываем старые DNS на всех интерфейсах
            for iface in interfaces:
                checker.exec_command(f'resolvectl dns {iface} "" 2>/dev/null')
            # Устанавливаем новые DNS на каждый интерфейс
            for iface in interfaces:
            ...
            lines.append("Не найдены интерфейсы для настройки через resolvectl.")

        # Устанавливаем глобальные DNS
        cmd_global = 'resolvectl dns ' + ' '.join(nameservers)
        out, err = checker.exec_command(cmd_global + ' 2>&1')
        if err.strip() and 'error' in err.lower():
            lines.append(f"❌ Ошибка глобальных DNS: {err.strip()}")
        else:
            lines.append(f"✅ Глобальные DNS установлены: {', '.join(nameservers)}")

        # Обновляем /etc/resolv.conf (синхронизация)
        lines.append("Обновляем /etc/resolv.conf для синхронизации.")
    else:
        lines.append("systemd-resolved не активен, редактируем /etc/resolv.conf напрямую.")

    # --- Единый блок перезаписи /etc/resolv.conf (делаем всегда) ---
    ...
    cmd_write = f'echo "{new_content}" > /etc/resolv.conf'
    out, err = checker.exec_command(cmd_write + ' 2>&1')
    if err.strip():
        lines.append(f"❌ Ошибка записи /etc/resolv.conf: {err.strip()}")
    else:
        lines.append("✅ /etc/resolv.conf обновлён.")

    # --- Перезапускаем systemd-resolved, если активен ---
    if out.strip() == 'active':
        lines.append("Перезапускаем systemd-resolved для применения изменений...")
        checker.exec_command('systemctl restart systemd-resolved 2>/dev/null')
        lines.append("✅ systemd-resolved перезапущен.")

    # --- Проверка результата ---
    out_check, _ = checker.exec_command('resolvectl dns 2>/dev/null')
    if out_check.strip():
        lines.append("\nТекущие DNS (resolvectl):\n" + out_check)
    out_check2, _ = checker.exec_command('cat /etc/resolv.conf')
