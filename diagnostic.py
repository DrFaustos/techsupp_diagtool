...
def set_dns_resolvers(checker, nameservers):
    ...
    if out.strip() == 'active':
        lines.append("Обнаружен systemd-resolved. Будем использовать resolvectl.")
        # Используем resolvectl для установки DNS
        # Определяем интерфейс по умолчанию (обычно eth0 или ens*)
        # Но можно задать глобальные DNS
        cmd = f'resolvectl dns {{}} ' + ' '.join(nameservers)
        # Проверяем, есть ли интерфейс по умолчанию
        out_iface, _ = checker.exec_command('ip route show default | awk \'{print $5}\' | head -1')
        default_iface = out_iface.strip()
        if default_iface:
            cmd = f'resolvectl dns {default_iface} ' + ' '.join(nameservers)
        else:
            # Глобальные
            cmd = 'resolvectl dns ' + ' '.join(nameservers)
        out, err = checker.exec_command(cmd + ' 2>&1')
        if err.strip() and 'error' in err.lower():
            lines.append(f"❌ Ошибка при установке DNS через resolvectl: {err.strip()}")
            lines.append("Попробуем прямой запись в /etc/resolv.conf")
        else:
            lines.append("✅ DNS установлены через resolvectl.")
            # Проверим
            out_check, _ = checker.exec_command('resolvectl dns')
            lines.append("Текущие DNS (resolvectl):\n" + out_check)
            return "\n".join(lines)
    
    # Если resolvectl не сработал или не активен, редактируем /etc/resolv.conf напрямую
    lines.append("Редактируем /etc/resolv.conf напрямую (создаём резервную копию).")
