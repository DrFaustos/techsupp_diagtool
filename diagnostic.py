...
def set_dns_resolvers(checker, nameservers):
    ...
    if out.strip() == 'active':
        lines.append("Обнаружен systemd-resolved. Используем resolvectl для интерфейсов и обновляем /etc/resolv.conf.")
        ...
        if interfaces:
            # Сброс и установка на интерфейсы
            ...
            lines.append("Не найдены интерфейсы для настройки DNS через resolvectl.")

        # Устанавливаем глобальные DNS
        cmd_global = 'resolvectl dns ' + ' '.join(nameservers)
        out, err = checker.exec_command(cmd_global + ' 2>&1')
        if err.strip() and 'error' in err.lower():
            lines.append(f"❌ Ошибка установки глобальных DNS: {err.strip()}")
        else:
            lines.append(f"✅ Глобальные DNS установлены: {', '.join(nameservers)}")

        # Перезаписываем /etc/resolv.conf, чтобы синхронизировать
    ...
    # Если resolvectl не активен, редактируем /etc/resolv.conf напрямую (как раньше)
